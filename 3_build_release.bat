...
REM Genera: "SudokuJ 2.2.0 Java" (JAR) y "SudokuJ 2.2.0 Windows" (EXE)
REM
REM ANTES de ejecutar: compila con 1_compile_windows.bat y 2_crear_exe.bat

setlocal enabledelayedexpansion

set VERSION=2.2.0
set DIR_JAR=SudokuJ 2.2.0 Java
set DIR_EXE=SudokuJ 2.2.0 Windows
set JAR_SOURCE=target\sudokuj-2.2.0.jar
...
if not exist "%JAR_SOURCE%" (
    ...
    echo.
    echo Ejecuta primero: 1_compile_windows.bat
    pause
    exit /b 1
)

REM === PASO 2: Crear directorio "SudokuJ 2.2.0 Java" para ZIP ===
echo [1/2] Creando directorio "%DIR_JAR%"...
if exist "%DIR_JAR%" rmdir /s /q "%DIR_JAR%"
mkdir "%DIR_JAR%"

copy "%JAR_SOURCE%" "%DIR_JAR%\sudokuj-2.2.0.jar" >nul
...
if %EXE_FOUND%==1 (
    ...
    echo           Ejecuta 2_crear_exe.bat para generarlo.
...
echo   1. Comprime "%DIR_JAR%" como SudokuJ-2.2.0-Java.zip
echo   2. Comprime "%DIR_EXE%" como SudokuJ-2.2.0-Windows.zip
echo.
