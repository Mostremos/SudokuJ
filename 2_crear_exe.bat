...
set JAR=target\sudokuj-2.2.0.jar
...
if defined LAUNCH4J (
    ...
    (
        ...
        echo   ^</jre^>
        echo   ^<versionInfo^>
        echo     ^<fileVersion^>2.2.0.0^</fileVersion^>
        echo     ^<txtFileVersion^>2.2.0^</txtFileVersion^>
        echo     ^<fileDescription^>SudokuJ - Juego de Sudoku^</fileDescription^>
        echo     ^<copyright^>GPL v2^</copyright^>
        echo     ^<productVersion^>2.2.0.0^</productVersion^>
        echo     ^<txtProductVersion^>2.2.0^</txtProductVersion^>
...
(
    ...
    echo cd /d "%%~dp0.."
    echo start "" java -jar "target\sudokuj-2.2.0.jar"
) > "%DEST%\%EXE_NAME%.bat"
